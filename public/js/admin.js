...
function fetchAssociatedRecords(relationName, updateId) {
    ...
    http.setRequestHeader('frameworkToken', token);
...
if (typeof drawComments == 'boolean') {
    ...
    function submitComment() {
        ...
        if (comment == "") {
            ...
            var targetUrl = baseUrl + 'api/create/framework_comments';
            const http = new XMLHttpRequest();
            http.open('post', targetUrl);
            http.setRequestHeader('Content-type', 'application/json');
            http.setRequestHeader('frameworkToken', token);
            ...
            http.onload = function() {
                ...
                if (http.status == 401) {
                    //invalid token!
                    window.location.href = baseUrl + 'framework_administrators/login';
    ...
    function fetchComments() {
        ...
        var targetUrl = baseUrl + 'api/get/framework_comments';
        const http = new XMLHttpRequest();
        http.open('post', targetUrl);
        http.setRequestHeader('Content-type', 'application/json');
        http.setRequestHeader('frameworkToken', token);
    ...
    function ditchPreviewPic() {
        ...
        var removePicUrl = baseUrl + 'framework_filezone/upload/' + segment1 + '/' + updateId;
