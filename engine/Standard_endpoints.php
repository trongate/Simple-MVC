<?php
class Standard_endpoints extends Framework {
    ...
    public function make_sure_allowed($target_endpoint, $table_name) {
        ...
        $framework_user = $this->framework_tokens->_get_user_obj($token);

        if($framework_user === false) {
            // Test for aaa token
        ...
        $user_level = $framework_user->user_level ?? '';
        $framework_user_id = $framework_user->framework_user_id ?? '';
        $framework_user_code = $framework_user->framework_user_code ?? '';
        ...
        if (isset($endpoint_auth_rules['userIds'])) {
            if (in_array($framework_user_id, $endpoint_auth_rules['userIds'])) {
                return $token; //match for allowed framework_user_id
            }
        }

        if(segment(1) === 'api') {

            //user id segment authorization (only for Framework API paths!)
            if (isset($endpoint_auth_rules['userIdSegment'])) {
                $target_value = segment($endpoint_auth_rules['userIdSegment']);
                if ($framework_user_id === $target_value) {
                    return $token; //match for framework_user_id on target segment
                }
            }

            //user code segment authorization (only for Framework API paths!)
            if (isset($endpoint_auth_rules['userCodeSegment'])) {
                $target_value = segment($endpoint_auth_rules['userCodeSegment']);
                if ($framework_user_code === $target_value) {
                    return $token; //match for framework_user_code on target segment
                }
            }

            //user owned segment authorization (only for Framework API paths!)
            if (isset($endpoint_auth_rules['userOwnedSegment'])) {
                $test_data['table_name'] = $table_name;
                $test_data['framework_user_id'] = $framework_user_id;
    ...
    private function test_for_aaa_token($token) {
        $token_obj = $this->model->get_one_where('token', $token, 'framework_tokens');
        $code = $token_obj->code ?? '';
    ...
    private function run_user_owned_test($test_data) {
        ...
        if ($record === false) {
            ...
            $framework_user_id = $test_data['framework_user_id'] ?? 0;
            $target_framework_user_id = $record->framework_user_id ?? 0;
            settype($framework_user_id, 'int');
            settype($target_framework_user_id, 'int');

            if ($framework_user_id === $target_framework_user_id && ($framework_user_id>0)) {
