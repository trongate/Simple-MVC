...
class Core {
    ...
    private function serve_module_asset() {
        ...
        foreach ($url_segments as $url_segment_key => $url_segment_value) {
            $pos = strpos($url_segment_value, $this->module_assets_trigger);

            if (is_numeric($pos)) {
                $target_module = str_replace($this->module_assets_trigger, '', $url_segment_value);
                ...
                $asset_path = '../controllers/'.strtolower($target_module).'/assets/'.$target_dir.'/'.$file_name;
    ...
    private function serve_controller() {
        ...
        $controller_path = '../controllers/'.$this->current_controller.'.php';

        if ($controller_path === '../controllers/api/controllers/Api.php') {
    ...
    private function attempt_init_child_controller($controller_path) {
        ...
        if (count($bits)==2) {
            if (strlen($bits[1])>0) {
                ...
                $controller_path = '../controllers/'.$parent_module.'/'.$child_module.'/controllers/'.ucfirst($bits[1]).'.php';
    ...
    <body>
        ...
        <p>This page is being generated by the Framework Framework.</p>
        <p>To edit this page, go to: templates/views/error_404.php</p>
        <p>To explore the documentation go to <a href="http://www.framework.io/documentation">http://www.framework.io/documentation</a></p>
