...
class Api extends Framework {
    ...
    private function generate_new_golden_token() {
            ...
            $sql = 'delete from framework_tokens where user_id = :user_id and code = :code';
            $this->model->query_bind($sql, $token_data);

            $this->module('framework_tokens');
            $token_data['expiry_date'] = time() + 7200; //two hours
            $golden_token = $this->framework_tokens->_generate_token($token_data);
            return $golden_token;
    }
    ...
    public function validate_token($token_validation_data) {
        ...
        $table_name = segment(1) === 'api' || segment(1) === 'framework_filezone' ? segment(3) : segment(1);
